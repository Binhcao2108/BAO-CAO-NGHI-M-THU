...
function MultiSelect({
  ...
  const displayText =
    selected.length === 0
      ? title
      : selected.length === 1
        ? selected[0].length > 20
          ? selected[0].substring(0, 20) + "..."
          : selected[0]
          ...
          <div className="overflow-y-auto p-1 max-h-56 custom-scrollbar">
            ...
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map((opt: any) => (
                <label
                  key={opt.name}
                  className="flex items-center gap-2 px-2.5 py-1.5 text-xs hover:bg-gray-100 rounded-lg cursor-pointer text-gray-700 transition-colors"
                >
                  <input
                    type="checkbox"
                    ...
                    className="rounded border-gray-300 text-blue-500 focus:ring-blue-500/50"
                  />
                  <span className="truncate max-w-[170px]" title={opt.name}>
                    {opt.name.length > 25
                      ? opt.name.substring(0, 25) + "..."
                      : opt.name}
                  </span>
                  <span className="text-gray-400 font-mono text-[10px] ml-auto shrink-0">
                    ({opt.count})
                  </span>
                </label>
              ))
...
const getErrorTags = (text: string, row?: any): string[] => {
  ...
  if (text && text !== "N/A" && text !== "Chưa phát hiện lỗi") {
    ...
    if (
      ...
      lower.includes("quá nhiệt")
    )
      tags.push("Quá nhiệt");
    if (
      lower.includes("treo") ||
      lower.includes("reboot") ||
      lower.includes("khởi động")
    )
      tags.push("Treo / Reboot");
    if (
      lower.includes("rssi") ||
      lower.includes("wifi yếu") ||
      lower.includes("wifi kém")
    )
    ...
    if (
      lower.includes("lan 100") ||
      lower.includes("nhận 100") ||
      lower.includes("dây lan") ||
      lower.includes("lan")
    )
      tags.push("Lỗi dây LAN / Cổng 100M");
    if (lower.includes("mesh")) tags.push("Lỗi Mesh");
    if (lower.includes("cáp") || lower.includes("đứt"))
      tags.push("Đứt / Lỗi Cáp");
    if (
      lower.includes("xong") ||
      lower.includes("hoàn thành") ||
      lower.includes("done")
    )
      tags.push("Đã xử lý (Xong)");
    if (lower.includes("chưa") || lower.includes("đang"))
      tags.push("Đang hoặc Chưa xử lý");
  }
  ...
  if (row) {
    ...
    Object.keys(row).forEach((key) => {
      ...
      if (isTrue(row[key])) {
        if (
          kLower.includes("weakrssi") ||
          kLower.includes("wifi kém") ||
          kLower.includes("wifi yếu") ||
          kLower.includes("ổn định wifi")
        ) {
          tags.push("Wifi yếu (RSSI)");
        }
...
const ErrorDetail = ({
  ...
  if (!text || text === "N/A" || text === "Chưa phát hiện lỗi")
    return <span className="text-gray-500 italic">Chưa phát hiện lỗi</span>;
  ...
  return (
    <div className="flex flex-col gap-2 w-full max-w-[350px]" title={text}>
      <span
        className={`text-xs leading-relaxed font-sans ${isRose ? "text-rose-800" : "text-amber-800"}`}
      >
        {cleanDesc}
      </span>
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {tags.map((t) => (
            <span
              key={t}
              className={`px-2 py-0.5 border rounded text-[10px] font-semibold uppercase tracking-wider whitespace-nowrap shadow-sm ${
                isRose
...
export default function TelecomDashboard({ sheet, filteredData }: Props) {
  ...
  const [statusChartType, setStatusChartType] = useState<"list" | "chart">(
    "list",
  );
  ...
  const telecomData = useMemo(() => {
    ...
    const repeatCol = findColumnByKeywords(cols, COL_KEYWORDS.repeat);
    ...
    filteredData.forEach((row, idx) => {
      ...
      const region =
        regionCol && row[regionCol] ? String(row[regionCol]) : "Unknown";
      const user =
        accountCol && row[accountCol] ? String(row[accountCol]) : `Cus-${idx}`;
      const modem =
        modemCol && row[modemCol] ? String(row[modemCol]) : "Unknown";
      const team = teamCol && row[teamCol] ? String(row[teamCol]) : "N/A";
      const repeat =
        repeatCol && row[repeatCol] ? String(row[repeatCol]) : "N/A";

      const rawDetectedError =
        warningCol && row[warningCol]
          ? String(row[warningCol])
          : causeCol && row[causeCol]
            ? String(row[causeCol])
            : "Chưa phát hiện lỗi";
      ...
      repeatCount[repeat] = (repeatCount[repeat] || 0) + 1;

      const detectTags = getErrorTags(rawDetectedError || detectedError, row);
      if (detectTags.length === 0) {
        detectCount["Chưa phát hiện lỗi"] =
          (detectCount["Chưa phát hiện lỗi"] || 0) + 1;
      } else {
      ...
      const realStatus =
        rawStatusValue !== "Chưa phát hiện lỗi" && rawStatusValue
          ? String(rawStatusValue).trim()
          : "Chưa phát hiện lỗi";
      const statusTags =
        realStatus !== "Chưa phát hiện lỗi" && realStatus !== ""
          ? [realStatus]
          : [];
      ...
      if (realStatus !== "Chưa phát hiện lỗi" && realStatus !== "") {
        ...
        statusCount["Chưa phát hiện lỗi"] =
          (statusCount["Chưa phát hiện lỗi"] || 0) + 1;
      }
    ...
    return {
      ...
      modemData,
      actionList,
      availableModems: Object.entries(modemCount)
        .sort((a, b) => b[1] - a[1])
        .map(([name, count]) => ({ name, count })),
      availableCauses: Object.entries(causesCount)
        .sort((a, b) => b[1] - a[1])
        .map(([name, count]) => ({ name, count })),
      availableDetectTags: Array.from(uniqueDetectTags)
        .sort()
        .concat(detectCount["Chưa phát hiện lỗi"] ? ["Chưa phát hiện lỗi"] : [])
        .map((name) => ({ name, count: detectCount[name] || 0 })),
      availableStatusTags: Array.from(uniqueStatusTags)
        .sort()
        .concat(statusCount["Chưa phát hiện lỗi"] ? ["Chưa phát hiện lỗi"] : [])
  ...
  const filterPredicates: Record<string, (a: any) => boolean> = useMemo(
    () => ({
      status: (a: any) => {
        ...
        const s =
          a.statusTags.length > 0 && a.statusTags[0] !== "Chưa phát hiện lỗi"
            ? a.statusTags[0]
            : "Chưa phát hiện lỗi";
      ...
      detect: (a: any) => {
        ...
        if (
          filterDetect.includes("Chưa phát hiện lỗi") &&
          a.detectTags.length === 0
        )
          return true;
      ...
      repeat: (a: any) => {
        if (filterRepeat.length === 0) return true;
        return filterRepeat.includes(a.repeat);
      },
    }),
    [
      filterStatus,
  ...
  const displayedActions = useMemo(() => {
    return telecomData.actionList.filter((a) =>
      Object.values(filterPredicates).every((p) => p(a)),
    );
  ...
  const dynRepeatOptions = useMemo(() => {
    const counts: Record<string, number> = {};
    const filteredData = getFilteredDataWithout("repeat");
    filteredData.forEach((a) => {
      counts[a.repeat] = (counts[a.repeat] || 0) + 1;
    });
    return Object.entries(counts)
      .map(([name, count]) => ({ name, count }))
      .sort((a, b) => a.name.localeCompare(b.name));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 mb-2 relative z-10">
            ...
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              Tổng Thiết bị / Thuê bao
            </h3>
          </div>
          <div className="text-3xl font-bold text-gray-900 font-mono mt-1 relative z-10">
            {telecomData.total.toLocaleString()}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 mb-2 relative z-10">
            ...
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              Dòng Modem Nhiều Nhất
            </h3>
          </div>
          <div
            className="text-lg font-bold text-gray-900 mt-2 relative z-10 truncate"
            title={topModem}
          >
            {topModem}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-800 mb-6 flex items-center gap-2">
            <AlertCircle size={16} className="text-rose-500" /> Nhóm nguyên nhân
            thường gặp
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={telecomData.paretoCauses}
                layout="vertical"
              ...
              >
                ...
                <Bar
                  ...
                  maxBarSize={40}
                  onClick={(data) => {
                    setFilterCause([data.originalName || data.name]);
                    setFilterDetect([]);
                    setFilterModem([]);
                  }}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-800 mb-6 flex items-center gap-2">
            <AlertCircle size={16} className="text-rose-500" /> Nhóm lỗi detect
            thường gặp
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={telecomData.paretoDetects}
                layout="vertical"
                margin={{ top: 5, right: 30, left: 10, bottom: 5 }}
              >
                <CartesianGrid
                ...
                <Bar
                  ...
                  maxBarSize={40}
                  onClick={(data) => {
                    setFilterDetect([data.originalName || data.name]);
                    setFilterModem([]);
                  }}
                  className="cursor-pointer"
                />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-gray-800">
                    Tình trạng lỗi ban đầu
                  </h3>
                  <p className="text-[11px] text-gray-500 mt-0.5">
                    Nhấn để lọc nhanh danh sách
                  </p>
                </div>
              </div>
            ...
            {statusChartType === "list" ? (
              <div className="space-y-3.5 max-h-[290px] overflow-y-auto pr-1 custom-scrollbar">
                {telecomData.paretoStatus.map((item, index) => {
                  const maxVal =
                    Math.max(...telecomData.paretoStatus.map((s) => s.count)) ||
                    1;
                  const percentage = Math.round(
                    (item.count / telecomData.total) * 100,
                  );
                  const barWidth = `${(item.count / maxVal) * 100}%`;
                  const isSelected = filterStatus.includes(
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <span
                          className={`text-xs font-semibold leading-relaxed break-words flex-1 ${isSelected ? "text-amber-900 font-bold" : "text-gray-700 group-hover:text-amber-800"}`}
                        >
                          {item.originalName || item.name}
                        </span>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-800 mb-6 flex items-center gap-2">
            <ServerCrash size={16} className="text-blue-500" /> Phân loại theo
            Modem (Thiết bị)
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={telecomData.modemData}
              ...
              >
                ...
                <Bar
                  ...
                  maxBarSize={40}
                  onClick={(data) =>
                    setFilterModem([data.originalName || data.name])
                  }
                  className="cursor-pointer"
                />
              </BarChart>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-800 mb-6 flex items-center gap-2">
            <Users size={16} className="text-emerald-500" /> Tần suất theo Khu
            Vực / Block
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={telecomData.regionData}
                margin={{ top: 20, right: 30, left: 20, bottom: 60 }}
              >
                <CartesianGrid
                  strokeDasharray="3 3"
                ...
                <Bar
                  ...
                  maxBarSize={40}
                  onClick={(data) =>
                    setFilterRegion([data.originalName || data.name])
                  }
                  className="cursor-pointer"
                />
              </BarChart>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-800 mb-6 flex items-center gap-2">
            <Users size={16} className="text-indigo-500" /> Tần suất theo KTV
            (Nhân sự / Team)
          </h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={telecomData.teamData}
                layout="vertical"
                margin={{ top: 5, right: 30, left: 10, bottom: 5 }}
              >
                <CartesianGrid
                ...
                <Bar
                  ...
                  maxBarSize={40}
                  onClick={(data) =>
                    setFilterTeam([data.originalName || data.name])
                  }
                  className="cursor-pointer"
                />
              </BarChart>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <h3 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
            <ArrowRight size={16} className="text-blue-500" /> Danh sách thiết
            bị / thuê bao lỗi ({displayedActions.length} kết quả)
          </h3>
          <div className="flex flex-wrap items-center gap-3">
            ...
            />
            <MultiSelect
              title="Tất cả Lặp/Repeat"
              options={dynRepeatOptions}
              selected={filterRepeat}
              onChange={setFilterRepeat}
              className="w-full sm:w-48"
            />
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-200 text-gray-500 text-xs uppercase tracking-wider">
                ...
                <th className="pb-3 px-4 font-medium">CL Lặp</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {displayedActions.map((action, i) => (
                ...
                >
                  <td className="py-3 px-4 text-gray-900 font-mono text-xs align-top whitespace-nowrap">
                    {action.user}
                  </td>
                  <td className="py-3 px-4 text-gray-600 text-xs align-top">
                    {action.region}
                  </td>
                  <td className="py-3 px-4 text-gray-600 text-xs align-top whitespace-nowrap">
                    {action.team}
                  </td>
                  <td className="py-3 px-4 text-gray-600 text-xs align-top whitespace-nowrap">
                    {action.repeat}
                  </td>
                  <td className="py-3 px-4 text-blue-600 font-mono text-xs align-top min-w-[120px]">
                    {action.modem}
                  </td>
                  <td className="py-3 px-4 align-top">
                    <ErrorDetail
                      text={action.detectedError}
                      tags={action.detectTags}
                      color="rose"
                    />
                  </td>
                  <td className="py-3 px-4 align-top">
